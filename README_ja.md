# ZeroTraceBrowser - ローカル写真ブラウザ、安全削除対応フォト整理ツール、重複写真ファインダー

[English](README.md) | [中文](README_zh.md)

> Windows とデスクトップユーザー向けの local-first なローカル写真ブラウザ兼画像整理ツールです。
> ローカルフォルダを直接閲覧し、EXIF メタデータを確認し、重複写真を見つけ、iPhone 写真を取り込み、クラウドアップロードや自動ファイル操作なしで、アプリ内リサイクル領域へ安全に削除できます。

---

## 概要

**ZeroTraceBrowser** は、FastAPI と Vanilla JavaScript で作られた軽量なローカル写真管理ツールです。ローカルフォルダ、外付けドライブ、NAS フォルダに写真を保存しているユーザーが、閲覧、コピー、削除、復元、重複写真の確認を明示的に制御できるように設計されています。
...
**現在の基準版:** `v0.5.0 stable baseline` — 2026-05-29

この基準版は、ZeroTraceBrowser の最初の安定して使えるリリースです。大量の写真を扱うローカルワークフローとして、ギャラリー閲覧、Timeline の高速ジャンプ、ビューアの初回画像表示優先、ビューアコンテキストのバックグラウンド補完、Timeline への復帰、重複写真の確認、アプリ内リサイクル領域、iPhone 取り込みが、制御しやすい一連の流れとしてつながりました。

今後の `v0.5.x` では、この安定基準版を中心に仕上げと堅牢化を進めます。

---

## 利用シーン

ZeroTraceBrowser は、次のような用途に向いています。

* 大量のローカル画像フォルダを扱う写真ブラウザ
* Windows 向けのオフライン画像ビューア
* 復元可能な削除に対応した安全な写真整理ツール
* 手動確認を前提にした重複写真ファインダー
* EXIF メタデータビューア
* ローカル写真アーカイブ向けの iPhone 写真取り込み補助ツール
* 軽量なセルフホスト型写真管理ツール
* クラウド写真管理サービスの代替となるローカル写真管理ツール
...
iPhone 取り込みページでは、iPhone の写真を現在の active image root に取り込めます。元ファイルの削除は明示的で制御された操作として扱います。

現在対応している経路:

* Windows iPhone MTP による小さな手動バッチ取り込み
* iPhone Shortcut によるローカルネットワーク経由のアップロード

Android 取り込みには対応していません。この機能は意図的に iPhone に限定しています。Android の MTP ストレージ構成はメーカーや機種によって差が大きいためです。
...
9. 必要に応じて、iPhone 取り込みページから iPhone 写真を取り込みます。
