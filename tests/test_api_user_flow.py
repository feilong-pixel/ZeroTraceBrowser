...
def test_images_timeline_neighbor_returns_adjacent_cached_group(api_client, monkeypatch) -> None:
    client, _, image_root, _ = api_client
    index_dir = ztb_context.root_image_index_dir(image_root)
    cache_key = image_scan_cache_key(
        image_root,
    ...
    save_image_index_cache(
        ...
        cache_key,
        [
            indexed_image("apr.jpg", "2024-04-01T00:00:00"),
            indexed_image("may.jpg", "2024-05-01T00:00:00"),
            indexed_image("jun.jpg", "2024-06-01T00:00:00"),
        ],
    )

    def fail_full_scan(*args, **kwargs):
        raise AssertionError("timeline neighbor should not scan the filesystem")

    monkeypatch.setattr(image_scan_service, "list_lightweight_image_metadata", fail_full_scan)

    prev_response = client.get(
        "/api/images/timeline-neighbor",
        params={"group_key": "2024-05", "direction": "prev"},
    )
    assert prev_response.status_code == 200
    assert prev_response.json()["neighbor_group_key"] == "2024-06"

    next_response = client.get(
        "/api/images/timeline-neighbor",
        params={"group_key": "2024-05", "direction": "next"},
    )
    assert next_response.status_code == 200
    assert next_response.json()["neighbor_group_key"] == "2024-04"

    bad_response = client.get(
        "/api/images/timeline-neighbor",
        params={"group_key": "2024-05", "direction": "sideways"},
    )
