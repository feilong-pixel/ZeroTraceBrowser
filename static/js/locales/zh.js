...
export default {
  ...
  browser: {
    ...
    duplicates: {
      ...
      empty: "尚未发现可用的 duplicates.json 结果。",
  ...
  duplicates: {
    ...
    bulkConfirm: "确认移入",
    bulkDeleted: (count) => `已批量移入回收区：${count} 个文件`,
    bulkDeleting: (count) => `正在批量移入回收区：${count} 个文件`,
    bulkDisabledForPhash: "pHash 相似检测需逐组人工确认，不能批量删除。",
    bulkMoveToRecycle: "批量移入",
    bulkStrictTitle: (count) => `将移动 ${count} 个 strict 重复文件到回收区`,
    confirmBulkStrictDelete: (count) =>
          `将把 ${count} 个 strict 重复文件移入回收区。只会处理 duplicate 文件，不会删除 kept 保留文件。是否继续？`,
    deleteSelected: "删除",
    deleted: (path) => `已删除 ${path}`,
    deleting: (path) => `正在删除 ${path} ...`,
    ...
    noStrictDuplicatesToDelete: "当前 strict 结果中没有可批量删除的重复文件。",
    openTasksTool: "整理工具",
  ...
  recycle: {
    ...
    backToGallery: "返回主画面",
    clearRecycle: "清空",
    cleared: (count) => `已清空 ${count} 个回收区文件。`,
    ...
    confirmClear: {
      confirm: "清空回收区",
      message: "确认清空回收区吗？此操作无法撤销。",
      messagePermanent: "确认清空回收区吗？当前系统下文件将被彻底删除，无法恢复。",
      messageSystemRecycle: "确认清空回收区吗？文件将被放入 Windows 系统回收站。",
      title: "清空回收区"
    },
    confirmClearLogs: {
      confirm: "清理日志",
