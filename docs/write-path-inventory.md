...
| `POST /api/delete` | Moves active-root media to `deleted/`; removes stale thumbnail; appends `logs/delete_log.csv`. | Updates Index data in `image_items`, `image_indexes`, and `timeline_entries`; marks matching `duplicate_items` unavailable for the duplicate page; upserts `recycle_records` for the recycle page. | Clears active-root image list cache; returns the recomputed Index image total. | This is the key safe-delete path for visible pages; originals are not unlinked directly, and hash/EXIF/similarity caches are left to explicit rebuild/maintenance paths. |
